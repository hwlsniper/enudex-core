...
#include <enudex/chain/block_database.hpp>
...
namespace enudex { namespace debug_witness {
...
{
   public:
      debug_api_impl( enudex::app::application& _app );
      ...
      std::shared_ptr< enudex::debug_witness_plugin::debug_witness_plugin > get_plugin();

      enudex::app::application& app;
};

debug_api_impl::debug_api_impl( enudex::app::application& _app ) : app( _app )
{}
...
void debug_api_impl::debug_push_blocks( const std::string& src_filename, uint32_t count )
{
   ...
   std::shared_ptr< enudex::chain::database > db = app.chain_database();
   ...
   if( fc::is_directory( src_path ) )
   {
      ...
      enudex::chain::block_database bdb;
      bdb.open( src_path );
      uint32_t first_block = db->head_block_num()+1;
      for( uint32_t i=0; i<count; i++ )
      {
         fc::optional< enudex::chain::signed_block > block = bdb.fetch_by_number( first_block+i );
         if( !block.valid() )
         {
...
void debug_api_impl::debug_generate_blocks( const std::string& debug_key, uint32_t count )
{
   ...
   fc::optional<fc::ecc::private_key> debug_private_key = enudex::utilities::wif_to_key( debug_key );
   FC_ASSERT( debug_private_key.valid() );
   enudex::chain::public_key_type debug_public_key = debug_private_key->get_public_key();

   std::shared_ptr< enudex::chain::database > db = app.chain_database();
   for( uint32_t i=0; i<count; i++ )
   {
      enudex::chain::witness_id_type scheduled_witness = db->get_scheduled_witness( 1 );
      fc::time_point_sec scheduled_time = db->get_slot_time( 1 );
      enudex::chain::public_key_type scheduled_key = scheduled_witness( *db ).signing_key;
      ...
      db->generate_block( scheduled_time, scheduled_witness, *debug_private_key, enudex::chain::database::skip_nothing );
   }
}
...
std::shared_ptr< enudex::debug_witness_plugin::debug_witness_plugin > debug_api_impl::get_plugin()
{
   return app.get_plugin< enudex::debug_witness_plugin::debug_witness_plugin >( "debug_witness" );
}
...
debug_api::debug_api( enudex::app::application& app )
{
